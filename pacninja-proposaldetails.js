...
function budgetdetailsRefresh(useHash) {
    ...
    var query = '/api/governanceproposals?testnet=' + pacninjatestnet;
    if (useHash) {
        ...
        if ((!data.hasOwnProperty("data")) || (!data.data.hasOwnProperty("governanceproposals")) || (!Array.isArray(data.data.governanceproposals)) || (data.data.governanceproposals.length == 0)) {
            ...
            if (pacninjatxexplorer[pacninjatestnet].length > 0) {
                ...
                for (var i = 0, ien = pacninjatxexplorer[pacninjatestnet].length; i < ien; i++) {
                    if (ix == 0) {
                        outtxt += '<a href="' + pacninjatxexplorer[pacninjatestnet][0][0].replace('%%a%%', data.data.governanceproposals[0].CollateralHash) + '">' + data.data.governanceproposals[0].CollateralHash + '</a>';
                    } else {
                        outtxt += '<a href="' + pacninjatxexplorer[pacninjatestnet][i][0].replace('%%a%%', data.data.governanceproposals[0].CollateralHash) + '">[' + ix + ']</a>';
                    }
                    ix++;
                }
            } else {
            ...
            $('#budgetmonthlyamount').html(addCommas(data.data.governanceproposals[0].PaymentAmount.toFixed(3)) + ' ' + pacninjacoin[pacninjatestnet]);
            $('#budgettotalamount').html(addCommas((data.data.governanceproposals[0].PaymentAmount * monthsleft).toFixed(3)) + ' ' + pacninjacoin[pacninjatestnet]);
            ...
            if (pacninjaaddressexplorer[pacninjatestnet].length > 0) {
                ...
                for (var i = 0, ien = pacninjaaddressexplorer[pacninjatestnet].length; i < ien; i++) {
                    if (ix == 0) {
                        outtxt += '<a href="' + pacninjaaddressexplorer[pacninjatestnet][0][0].replace('%%a%%', data.data.governanceproposals[0].PaymentAddress) + '">' + data.data.governanceproposals[0].PaymentAddress + '</a>';
                    } else {
                        outtxt += '<a href="' + pacninjaaddressexplorer[pacninjatestnet][i][0].replace('%%a%%', data.data.governanceproposals[0].PaymentAddress) + '">[' + ix + ']</a>';
                    }
                    ix++;
                }
            } else {
            ...
            if (tableVotes !== null) {
                ...
                tableVotes = $('#votestable').dataTable({
                    ajax: {
                        url: '/api/governanceproposals/votes?testnet=' + pacninjatestnet + '&proposalhash=' + encodeURIComponent(currentbudget.Hash),
            ...
            if (tableSuperBlocks !== null) {
                ...
                tableSuperBlocks = $('#superblockstable').dataTable({
                    ajax: {
                        url: '/api/blocks/superblocks?testnet=' + pacninjatestnet + '&proposalshash=["' + encodeURIComponent(currentbudget.Hash) + '"]',
...
function refreshBudgetProjection(useHash) {
    ...
    $.getJSON(query, function(data) {
        ...
        if ((data.hasOwnProperty("data")) && (data.data.hasOwnProperty("budgetsprojection")) && (Array.isArray(data.data.budgetsprojection)) && (data.data.budgetsprojection.length == 1) &&
            ...
            $('#budgetstatus').html("Valid and expected next super block (" + addCommas(data.data.budgetsprojection[0].Alloted.toFixed(3)) + " " + pacninjacoin[pacninjatestnet] + ").");
        } else {
...
$(document).ready(function() {
    ...
    $('#superblockstable').on('xhr.dt', function(e, settings, json) {
        ...
        $('#budgettotalpaymentsdash').text(totalamount.toFixed(3) + ' ' + pacninjacoin[pacninjatestnet]);
