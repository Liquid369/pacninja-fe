...
// Pac Ninja Front-End (pacninja-fe) - Masternode Detail
    ...
    $.getJSON(query, function(data) {
        ...
        if ((!data.hasOwnProperty("data")) || (data.data.length < 1)) {
            ...
            if ((data.data[0].hasOwnProperty("Portcheck")) && (data.data[0].Portcheck != false)) {
                ...
                } else if ((data.data[0].Portcheck.SubVer.length > 11) && (data.data[0].Portcheck.SubVer.substring(0, 11) == '/Pac Core:') && (data.data[0].Portcheck.SubVer.substring(data.data[0].Portcheck.SubVer.length - 1) == '/')) {
        ...
        tablePayments = $('#paymentstable').dataTable({
            ...
            searching: false,
            ajax: {
                url: '/api/blocks?testnet=' + pacninjatestnet + '&pubkeys=["' + data.data[0].MasternodePubkey + '"]&interval=P1M',
                dataSrc: 'data.blocks',
                cache: true
            },
            paging: false,
